...
if check_password():
    st.markdown("""<style> 
        ...
        div.stButton > button { font-weight: bold; border: 1px solid #1A237E; }
    ...
    if data:
        ...
        with c1:
            ...
            st.write(f"BOLL 高轨: **${last_h['Upper']:.2f}**")
            st.write(f"BOLL 中轨: **${last_h['MA20']:.2f}**")
            st.write(f"BOLL 低轨: **${last_h['Lower']:.2f}**")
        ...
        with c2:
            ...
            st.table(pd.DataFrame({
                "场景": ["看空失效", "中性回归", "支撑测试"],
                "压力位": [p_h*1.06, p_h, p_h*0.94],
                "支撑位": [p_l*1.06, p_l, p_l*0.94]
        ...
        fig_k.add_trace(go.Scatter(x=p_df.index, y=p_df['MA20'], line=dict(color='rgba(0,102,204,0.8)', dash='dot'), name=f"BOLL Mid: {last_h['MA20']:.2f}"), row=1, col=1)
        ...
        with o_col:
            st.subheader(f"🕯️ 期权链 (到期: {mkt['exp']})")
            if not calls_df.empty:
                display_calls = calls_df[['strike', 'lastPrice', 'openInterest', 'impliedVolatility']].tail(5)
                display_calls.columns = ['行权价', '最新价', '未平仓', '隐波']
                st.dataframe(display_calls.style.format({'隐波': '{:.2%}'}), use_container_width=True)
            else:
                st.info("当前无可用期权数据")
        with d_col:
            st.subheader("🌑 大宗交易打印 (Dark Pool Print)")
        ...
        st.dataframe(hist_show[['Open', 'High', 'Low', 'Close', '换手', 'MFI', 'MA20', 'MA5']].style.format(subset=['Open', 'High', 'Low', 'Close', 'MFI', 'MA20', 'MA5'], precision=2), use_container_width=True)

    else:
        st.error("⚠️ 数据获取失败。可能原因：Yahoo 接口封锁中。请尝试更换 VPN 节点并等候 5 分钟。")
